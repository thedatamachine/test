...
validip = re.search(
    ...
    ip, re.I)  # regex to verify input is in a valid format
extractRange = re.match(r'\d*.\d*.\d*.(\d{1,3})-(\d{1,3})', ip, re.I)
extractTriplet = re.match(r'\d{1,3}.\d{1,3}.\d{1,3}.', ip, re.I)
